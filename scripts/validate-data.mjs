...
if (data.spotPosition?.lastKnown !== null && (!Number.isFinite(data.spotPosition?.lastKnown?.latitude) || !Number.isFinite(data.spotPosition?.lastKnown?.longitude) || Number.isNaN(Date.parse(data.spotPosition?.lastKnown?.dateTime)))) errors.push('spotPosition.lastKnown deve ser nulo ou conter posição e data válidas');
