...
test('Windy can read positions with a valid key but cannot access shore or import', {skip:!process.env.PORTALS_WORKER_BUNDLE}, async()=>{
 ...
 for(const origin of ['https://www.windy.com','https://windy.com']){
  ...
  assert.equal(r.status,200);assert.equal(r.headers.get('Access-Control-Allow-Origin'),origin);assert.deepEqual((await r.json()).history,[]);
