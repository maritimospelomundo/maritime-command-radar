...
async function sync(env:any){
 const sources=await Promise.allSettled([fetch(RADAR,{signal:AbortSignal.timeout(15000)}).then(async r=>{if(!r.ok)throw Error('radar');const d:any=await r.json();return [normalize(d.vesselApiPosition?.lastKnown,'VesselAPI'),normalize(d.marineTrafficPosition?.lastKnown,'MarineTraffic'),normalize(d.spotPosition?.lastKnown,'SPOT')].filter(Boolean)}),fetch(SPOT,{signal:AbortSignal.timeout(15000)}).then(async r=>{if(!r.ok)throw Error('spot');const d:any=await r.json();if(!Array.isArray(d.points))throw Error('spot');return d.points.map((p:any)=>normalize(p,'SPOT')).filter(Boolean)})]);
