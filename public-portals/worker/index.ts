...
async function sync(env:any){
 const sources=await Promise.allSettled([fetch(RADAR,{signal:AbortSignal.timeout(15000)}).then(async r=>{if(!r.ok)throw Error('radar HTTP '+r.status);const d:any=await r.json();return [['vesselApiPosition','VesselAPI'],['marineTrafficPosition','MarineTraffic'],['spotPosition','SPOT']].flatMap(([key,source])=>[...(Array.isArray(d[key]?.history)?d[key].history:[]),d[key]?.lastKnown].map(p=>normalize(p,source)).filter(Boolean))}),env.SPOT_WORKER.fetch(new Request(SPOT,{signal:AbortSignal.timeout(15000)})).then(async r=>{if(!r.ok)throw Error('spot HTTP '+r.status);const d:any=await r.json();if(!Array.isArray(d.points))throw Error('spot');return d.points.map((p:any)=>normalize(p,'SPOT')).filter(Boolean)})]);
